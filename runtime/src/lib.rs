...
use sp_core::{crypto::{KeyTypeId, Public}, u32_trait::*, OpaqueMetadata, H160, U256, H256};
use sp_runtime::{
...
pub use frame_support::{
	...
	traits::{KeyOwnerProofSystem, Randomness, StorageInfo, InstanceFilter, FindAuthor, OnUnbalanced, Imbalance},
	weights::{
...
use pallet_evm::{
	Account as EVMAccount, EnsureAddressTruncated, EnsureAddressNever, EnsureAddressRoot, FeeCalculator, HashedAddressMapping, Runner,
};
...
pub const MICROUNET: Balance = MILLIUNET / 1_000;
pub const NANOUNET: Balance = MICROUNET / 1_000;

pub const BYTE_FEE: Balance = 100 * MICROUNET;

pub const fn deposit(items: u32, bytes: u32) -> Balance {
	//items as Balance * 15 * MILLIUNET + (bytes as Balance) * 6 * MILLIUNET
	items as Balance * 1 * UNET + (bytes as Balance) * BYTE_FEE
}
...
parameter_types! {
	pub const ExistentialDeposit: Balance = 1 * UNET;
...
parameter_types! {
	pub const AssetDeposit: Balance = 100 * UNET;
	pub const ApprovalDeposit: Balance = 1 * UNET;
	pub const StringLimit: u32 = 50;
	pub const MetadataDepositBase: Balance = 10 * UNET;
	pub const MetadataDepositPerByte: Balance = 1 * UNET;
}
...
parameter_types! {
	pub const BasicDeposit: Balance = 5 * UNET;       // 258 bytes on-chain
	pub const FieldDeposit: Balance = 250 * MILLIUNET;        // 66 bytes on-chain
	pub const SubAccountDeposit: Balance = 1 * UNET;   // 53 bytes on-chain
...
pub struct UNETdGasWeightMapping;

impl pallet_evm::GasWeightMapping for UNETdGasWeightMapping {
...
impl pallet_evm::Config for Runtime {
	...
	type GasWeightMapping = UNETdGasWeightMapping;
...
parameter_types! {
	...
	pub const ProposalBondMinimum: Balance = 1 * UNET;
...
parameter_types! {
	...
	pub const MinimumDeposit: Balance = 4 * UNET;
	pub const MaxVotes: u32 = 100;
	pub const MaxProposals: u32 = 100;
	pub const PreimageByteDeposit: Balance = BYTE_FEE;
...
construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		TechComitteeCollective: pallet_collective::<Instance2>::{Pallet, Call, Storage, Event<T>, Origin<T>, Config<T>},		
