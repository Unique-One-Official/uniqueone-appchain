...
pub mod migrations {
	...
	>;
	pub type OldToken<T> = unet_orml_nft::TokenInfo<TokenIdOf<T>, OldTokenData>;
	pub type NewToken<T> = unet_orml_nft::TokenInfo<
		TokenIdOf<T>,
	...
	pub fn do_migrate<T: Config>() -> Weight {
		// migrate classes
		unet_orml_nft::Classes::<T>::translate::<OldClass<T>, _>(|_, p: OldClass<T>| {
...
pub mod module {
	...
	#[pallet::metadata(T::AccountId = "AccountId", ClassIdOf<T> = "ClassIdOf", TokenIdOf<T> = "TokenIdOf")]
	...
	impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
		fn build(&self) {
			...
			} in &self.classes
			{
				...
				{
					assert!(
						unet_orml_nft::Tokens::<T>::get(*class_id, *token_id).is_none(),
						"Dup token id"
					);
					...
					unet_orml_nft::NextTokenId::<T>::insert(*class_id, *token_id);
					unet_orml_nft::Pallet::<T>::mint(
						token_owner,
						*class_id,
				...
				unet_orml_nft::NextTokenId::<T>::insert(
					*class_id,
					max_token_id.checked_add(&One::one()).unwrap(),
				);
			}
			unet_orml_nft::NextClassId::<T>::set(max_class_id.checked_add(&One::one()).unwrap());
		}
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn update_token_royalty(
			...
			let who = ensure_signed(origin)?;
			unet_orml_nft::Tokens::<T>::try_mutate(
				class_id,
				token_id,
					...
					token_info.data.royalty_rate = charge_royalty
						.ok_or_else(|| -> Result<PerU16, DispatchError> {
							let class_info: ClassInfoOf<T> =
								unet_orml_nft::Pallet::<T>::classes(class_id)
		...
		pub fn mint(
			...
			let to = T::Lookup::lookup(to)?;
			let class_info: ClassInfoOf<T> =
				unet_orml_nft::Pallet::<T>::classes(class_id).ok_or(Error::<T>::ClassIdNotFound)?;
			let _ = Self::do_mint(
				&who,
				&to,
				&class_info,
		...
		pub fn destroy_class(
			...
			unet_orml_nft::Pallet::<T>::destroy_class(&who, class_id)?;
...
impl<T: Config> Pallet<T> {
	...
	fn do_mint(
		...
		let token_id: TokenIdOf<T> =
			unet_orml_nft::Pallet::<T>::mint(to, class_id, metadata, data, quantity)?;
	...
	pub fn do_create_class(
		...
		unet_orml_nft::Pallet::<T>::create_class(&owner, metadata, data)?;
	...
	pub fn do_transfer(
		...
		unet_orml_nft::Pallet::<T>::transfer(from, to, (class_id, token_id), quantity)?;
	...
	> {
		unet_orml_nft::Pallet::<T>::tokens(class_id, token_id).map(|t: TokenInfoOf<T>| {
			unet_traits::ContractTokenInfo {
				...
				data: unet_traits::ContractTokenData {
	...
	pub fn tokens_by_owner(
		...
		unet_orml_nft::TokensByOwner::<T>::iter_prefix(account_id)
			.map(|((c, t), a)| (c, t, a))
	...
	pub fn owners_by_token(
		...
		unet_orml_nft::OwnersByToken::<T>::iter_prefix((class_id, token_id))
			.map(|(account_id, _): (T::AccountId, ())| account_id)
			.skip(page.saturating_mul(page_size) as usize)
			.take(page_size as usize)
			.map(|account_id| {
				let at = unet_orml_nft::TokensByOwner::<T>::get(&account_id, (class_id, token_id))
					.unwrap_or_default();
				(account_id, at)
			})
			.collect()
...
impl<T: Config> unet_traits::UnetNft<T::AccountId, ClassIdOf<T>, TokenIdOf<T>> for Pallet<T> {
	fn peek_next_class_id() -> ClassIdOf<T> {
		unet_orml_nft::NextClassId::<T>::get()
	}
	...
	fn account_token(
		...
		unet_orml_nft::Pallet::<T>::tokens_by_owner(who, (class_id, token_id)).unwrap_or_default()
	}
	...
	fn reserve_tokens(
		...
		unet_orml_nft::Pallet::<T>::reserve(who, (class_id, token_id), quantity)
	}
	...
	fn unreserve_tokens(
		...
		unet_orml_nft::Pallet::<T>::unreserve(who, (class_id, token_id), quantity)
	}
