...
pub trait UnetNft<AccountId, ClassId, TokenId> {
...
where
	MultiCurrency:
		unet_orml_traits::MultiCurrency<AccountId, CurrencyId = CurrencyId, Balance = Balance>,
	NFT: UnetNft<AccountId, ClassId, TokenId>,
	ClassId: Copy,
	TokenId: Copy,
	CurrencyId: Copy,
...
macro_rules! nft_dbg {
	($($msg: expr),+ $(,)?) => {
		...
		println!($($msg),+);
		#[cfg(not(test))]
		log::log!(target: "unet", log::Level::Debug, $($msg),+);
	};
}
...
macro_rules! nft_info {
	($($msg: expr),+ $(,)?) => {
		...
		println!($($msg),+);
		#[cfg(not(test))]
		log::log!(target: "unet", log::Level::Info, $($msg),+);
	};
}
...
macro_rules! nft_err {
	($($msg: expr),+ $(,)?) => {
		...
		println!($($msg),+);
		#[cfg(not(test))]
		log::log!(target: "unet", log::Level::Error, $($msg),+);
	};
}
