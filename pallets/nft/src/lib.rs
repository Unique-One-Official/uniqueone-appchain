...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_class(
			...
			<T as pallet::Config>::Currency::transfer(&who, &owner, total_deposit, KeepAlive)?;

			<T as pallet::Config>::Currency::reserve_named(&RESERVE_ID, &owner, deposit)?;
		...
		pub fn destroy_class(
			...
			<T as pallet::Config>::Currency::unreserve_named(&RESERVE_ID, &who, data.deposit);
			...
			<T as pallet::Config>::Currency::transfer(
				&who,
				&dest,
				<T as pallet::Config>::Currency::free_balance(&who),
				AllowDeath,
			)?;
...
impl<T: Config> Pallet<T> {
	...
	fn do_transfer(from: &T::AccountId, to: &T::AccountId, token: (ClassIdOf<T>, TokenIdOf<T>)) -> DispatchResult {
		...
		<T as pallet::Config>::Currency::unreserve_named(&RESERVE_ID, &from, token_info.data.deposit);
		<T as pallet::Config>::Currency::transfer(&from, &to, token_info.data.deposit, AllowDeath)?;
		<T as pallet::Config>::Currency::reserve_named(&RESERVE_ID, &to, token_info.data.deposit)?;
	...
	fn do_mint(
		...
		// `DeadAccount`.
		<T as pallet::Config>::Currency::transfer(&who, &to, total_deposit, KeepAlive)?;
		<T as pallet::Config>::Currency::reserve_named(&RESERVE_ID, &to, total_deposit)?;
	...
	fn do_burn(who: T::AccountId, token: (ClassIdOf<T>, TokenIdOf<T>), remark: Option<Vec<u8>>) -> DispatchResult {
		...
		<T as pallet::Config>::Currency::unreserve_named(&RESERVE_ID, &who, token_info.data.deposit);
